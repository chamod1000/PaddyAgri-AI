...
# Streamlit cache_resource for cloud-friendly model caching
try:
    import streamlit as st
    _st_cache = st.cache_resource
except Exception:
    def _st_cache(func): return func
...
@_st_cache
def _load_embeddings_cached() -> HuggingFaceEmbeddings:
    """
    Loads the HuggingFace embedding model with Streamlit Cloud caching.
    Decorated with st.cache_resource so the model is downloaded once
    and reused across all reruns and users on Streamlit Cloud.
    """
...
def get_embeddings_model() -> HuggingFaceEmbeddings:
    """
    Returns the cached embedding model singleton.
    On Streamlit Cloud, uses st.cache_resource for container-level caching.
    """
    global _CACHED_EMBEDDINGS
    if _CACHED_EMBEDDINGS is not None:
        return _CACHED_EMBEDDINGS
    _CACHED_EMBEDDINGS = _load_embeddings_cached()
    return _CACHED_EMBEDDINGS
